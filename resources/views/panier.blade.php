...
$watch('darkMode', value => localStorage.setItem('darkMode', JSON.stringify(value)))" :class="{ 'b eh': darkMode === true }">
    ...
    <div class="bg-gray-900 text-white py-6">
        <div class="container mx-auto flex flex-col lg:flex-row justify-between items-center px-4">
            <h1 class="text-2xl font-bold mb-2 lg:mb-0">Panier</h1>
            <nav class="text-sm">
                <ol class="flex space-x-2">
                    <li><a href="/" class="hover:underline">Accueil</a></li>
                    <li class="text-gray-400">/ Panier</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="container mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 py-4">
            <!-- Cart Items -->
            <div class="lg:col-span-2">
                <div class="bg-white p-6 shadow rounded-lg">
                    <ul class="divide-y divide-gray-200">
                        ...
                        <li class="py-4 flex items-center space-x-4  cartitems product_data  ">
                            <img src="{{ asset('assets/uploads/chapitre_images/'.$value->chapitre->image_url ) }}" alt="Chapitre image" class="w-20 h-20 object-cover rounded" />
                            <div class="flex-1">
                                <input type="hidden" value="{{ $value->chapitre->id }}" class="prod_id">
                                <h6 class="text-lg font-semibold">{{ $value->chapitre->titre }}</h6>
                            </div>
                            <div class="text-center">
                              
                                <button class="text-red-500 hover:text-red-700 delete-cart-item">
                            ...
                            <div class="text-right">
                                <span class="text-lg font-bold">{{ $value->chapitre->montant }} FCFA</span>
                               
                            </div>
                        </li>
                        @php $total += $value->chapitre->montant; @endphp
                        @endforeach
                    </ul>
                </div>
            </div>
                ...
                <div class="bg-white p-6 shadow rounded-lg">
                    ...
                    <div class="border-b pb-2 mb-2">
                        <div class="flex justify-between">
                            <span class="font-semibold">Total Achat</span>
                            <span class="font-bold">{{ $total }} FCFA</span>
                        </div>
                    </div>
                    @php
                    $remise = 0;
                    ...
                    @endphp
                    @if(count($cartitems) >= 3)
                    <p class="text-green-500">Remise (5%) : -{{ number_format($remise, 0, ',', ' ') }} FCFA</p>
                    <h5 class="text-green-600 font-semibold text-center">Vous avez obtenu une remise</h5>
                    @endif
                    <div class="mt-4">
                        <div id='button_payee'></div>
                        <button id="custom_button" class="w-full bg-blue-600 text-white py-2 rounded-lg font-semibold flex justify-between px-4">
                            Acheter maintenant
                            <span>{{ number_format($totalApresRemise, 0, ',', ' ') }} FCFA</span>
                        </button>
                        @php
